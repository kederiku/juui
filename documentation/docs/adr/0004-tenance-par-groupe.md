...
propre décision ([ADR-0013](./0013-filtre-de-tenance-dans-le-depot.md)) ; et le piège documenté
dans `tenancy.py` a été tranché par BACK-10c : ce qui alimente la variable de contexte n'est pas un
intergiciel mais une **dépendance FastAPI**, un `BaseHTTPMiddleware` en étant incapable. Enfin, `group_id` ne porte pas encore de clé
