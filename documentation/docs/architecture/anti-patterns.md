...
| Importer l'**intérieur** d'une feature frontend — un fichier de son sous-dossier             | La feature n'a plus de surface : n'importe lequel de ses fichiers devient un point d'entrée, et plus rien ne peut y bouger.                                | **Mécanique** — `import-x/no-restricted-paths`, famille 1 ([ADR-0028](../adr/0028-frontieres-de-features.md))                 |
| Faire dépendre `components/` ou `lib/` d'une feature                                         | Le shell d'une application devient inutilisable sans le domaine qu'il nomme, et la dépendance s'inverse.                                                   | **Mécanique** — même règle, famille 2                                                                                         |
| Importer une page depuis une feature, `components/` ou `lib/`                                | Le routage est la racine de composition d'une application : le consommer inverse la dépendance, et fait dépendre le transverse d'un domaine en deux sauts. | **Mécanique** — même règle, famille 3                                                                                         |
| Ranger dans `components/` un composant qui porte du métier, en le déclarant « transverse »   | Le `components/` plat se reconstitue, et c'est exactement ce que le découpage par domaine voulait éviter.                                                  | **Revue** — aucun outil ne juge de la présence de métier                                                                      |
