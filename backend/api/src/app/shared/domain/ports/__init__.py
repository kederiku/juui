...
BACK-04 a pose la place et le sens, chaque ticket apporte son contrat. Quatre
sont livres -- `cache.py` par BACK-14, `file_storage.py` par BACK-13,
`unit_of_work.py` et `repository.py` par BACK-06a ; seul `token_service.py`
reste une place reservee. Le module pilote `identity` montre les ports METIER,
...
fichier perdu ; `AbstractUnitOfWork` LEVE ET ANNULE -- un commit en echec
remonte, une sortie de bloc sans commit n'ecrit rien. Un port ne se contente
